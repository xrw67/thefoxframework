...
public:
	...
	/// @beirf 得到索引指定的字段的名称
	const String &fieldName(const int index) const
	{
		if (!res)
			return "";
	...
	/// @beirf 根据索引得到int类型的值
	/// @return 返回对应的值
	const int getInt(cosnt int index) const
	{ return atoi(getString(index)); }
	
	/// @beirf 根据索引得到unsigned int类型的值
	/// @return 返回对应的值
	const uint32 getUInt(const int index) const
	{ return (uint32)atoi(getString(index)); }
